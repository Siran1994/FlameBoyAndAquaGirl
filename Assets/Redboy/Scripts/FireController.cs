// DecompilerFi decompiler from Assembly-CSharp.dll class: FireController
...
public class FireController : MonoBehaviour
{
			...
			if (other.contacts.Length == 1)
			{
				...
				if (y < position.y)
				{
					...
					if (Mathf.Abs(normal.x) != 1f)
					{
						...
						if (other.gameObject.tag != "Fire" && other.gameObject.tag != "PlayerIce")
						{
	...
	{
		if (other.enabled && other.gameObject.tag != "box" && other.gameObject.tag != "watter" && other.gameObject.tag != "black")
		{
			if (other.contacts.Length == 1)
			{
				Vector2 point = other.contacts[0].point;
	...
	{
		if (other.gameObject.tag == "watter")
		{
			Die = true;
	...
	private void pVic()
	{
		...
		InputManager.IPmanager.willcall();
		Time.timeScale = 0f;
	}
